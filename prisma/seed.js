...
async function main() {
  ...
  try {
    // 1. Lấy giá trị Enum động từ database để tránh lỗi invalid enum
    ...
    // 2. Mã hóa mật khẩu "admin@1234" bằng bcrypt
    const defaultPasswordHash = bcrypt.hashSync("admin@1234", 10);

    // 3. Xóa tài khoản test cũ để tránh xung đột UNIQUE (email, phone)
    await client.query(`
      DELETE FROM "users" 
    ...
    // 4. Tạo mới tài khoản Admin và User
    await client.query(
      `
      INSERT INTO "users" ("email", "password", "full_name", "phone", "role", "status", "created_at", "updated_at")
      VALUES 
        ('admin@gmail.com', $1, 'Administrator', '0900444333', $2::text::users_role, $3::text::users_status, NOW(), NOW()),
        ('user@phonestore.com', $1, 'Test User', '0999888777', $4::text::users_role, $3::text::users_status, NOW(), NOW());
    `,
      [defaultPasswordHash, adminRoleVal, activeUserStatusVal, userRoleVal]
    );
